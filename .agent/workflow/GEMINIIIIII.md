---
description: Universal CA Project Standard (Zero-Runtime & Type-Safe Architecture)
---
...
2.  **Full Project Mapping**: Before creating any new file or feature, MUST perform a `list_dir` on root and `src/` to identify existing documentation (LORE.md, RULES.md) and asset structures to prevent redundancy.
3.  **Referans Alignment**: Always use established components as blueprints for new ones to maintain aesthetic consistency.
4.  **No Placeholders**: Never use generic colors or simple boxes. Every element must be "designed" from the first render.

// turbo-all

## 5. Strategic Calibration (The "Voice of Reason" Protocol)
- **Challenge the Prompt**: If the User's request leads to a suboptimal, low-quality, or technically flawed outcome (e.g., "make 3D assets with CSS code" or "mix incompatible styles"), you MUST PROPOSE the superior alternative immediately.
- **Fail-Safe Warning**: If a request is technically impossible or will result in failure (e.g., "bad aesthetics"), STOP, WARN the user, and provide a concrete "Best Practice" alternative before proceeding. Do not blindly follow instructions that degrade project quality.

## 6. Anti-Patterns & Code Quality
- **NO GOD OBJECTS**: Never create massive, monolithic files that do everything. Break down complex UIs into smaller, focused, single-purpose sub-components immediately.
- **Micro-Componentization**: If a component exceeds 150-200 lines or handles multiple distinct visual elements (e.g., Board, Hand, Slot, Card), it MUST be split into separate files.
